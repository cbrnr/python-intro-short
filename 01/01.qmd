...
- On Windows, you should see a start menu entry inside the Python folder named "Python 3.12 (64-bit)" (or similar). Alternatively, you can launch Python from a regular command prompt: start "Command Prompt" or "Windows Terminal" and type `python` (followed by <kbd>Enter</kbd>).
...
:::{.callout-important}
The Python interpreter executable is called `python` on Windows and Linux, but `python3` on macOS. If you are using macOS, make sure to always use `python3` to start the Python interpreter.
:::
...
The interactive Python interpreter includes a *prompt*, which is typically the character sequence `>>>`. This prompt indicates that Python is ready to receive user input. You can type a Python command and hit <kbd>Enter</kbd> to run it. Python will immediately show the result of this command (if any) on the next line.

...
Python supports two types of loops (`for`-loops and `while`-loops), which enable us to perform repeated actions. Like functions and conditions, loops also control the program flow. Our previous example could be rewritten as follows:
...
:::{.callout-note}
Actually, `range` can also be called with two arguments, which are then interpreted as start and stop values of the created range. However, the stop value does not belong to the sequence anymore (this is because the difference between stop and start gives the number of elements):
...
If called with a third argument, `range` interprets this as a step value:
